...
MARKET_STUDY_PROMPTS: dict[str, str] = {
    ...
    "em.11.personas": (
        ...
        "ni moins de 2.\n"
        "STRUCTURE OBLIGATOIRE — sous-titres UNIQUES : chaque persona a son propre "
        "bloc, et chaque sous-titre de bloc est PREFIXE par le prenom (ou l'ID) "
        "du persona. Format impose :\n"
        "## Persona 1 — [Prenom]\n"
        "### [Prenom] — Contexte professionnel\n"
        "### [Prenom] — Motivations d'achat\n"
        "### [Prenom] — Freins et objections\n"
        "### [Prenom] — Parcours d'achat\n"
        "## Persona 2 — [Prenom2]\n"
        "### [Prenom2] — Contexte professionnel\n"
        "(et ainsi de suite)\n"
        "INTERDIT de repeter un sous-titre non prefixe entre les deux personas. "
        "Le gate refuse tout doublon de sous-titre dans le meme chapitre."
    ),
    ...
    "em.14.rentabilite_viabilite": (
        ...
        "REGLE ABSOLUE — COHERENCE CHIFFREE : chaque grandeur comptable (marge "
        "brute, seuil de rentabilite, EBE, CA previsionnel, resultat net) est "
        "citee avec UNE SEULE valeur numerique dans ce chapitre ET dans tout le "
        "document. Interdit d'ecrire successivement « marge brute 93 500 EUR » "
        "puis « marge brute 56 525 EUR » puis « marge brute 66 500 EUR » : le "
        "gate detecte les valeurs divergentes pour un meme libelle et refuse le "
        "livrable. Si tu explores plusieurs scenarios, nomme-les explicitement "
        "(« scenario prudent : X », « scenario cible : Y ») avec des libelles "
        "differents — jamais le meme libelle avec des valeurs differentes.\n"
        "REGLE — ANNONCES CHIFFREES : quand tu ecris « trois familles », « quatre "
        "segments », « cinq piliers », la liste ou l'enumeration qui suit doit "
        "avoir EXACTEMENT ce nombre d'items. Le gate compte les puces sous "
        "l'annonce et refuse tout desaccord."
    ),
    ...
    "em.22.sources": (
        ...
        "Documents client). Reprends en PRIORITE les URLs reelles du bloc "
        "SOURCES_WEB du contexte (titre + URL).\n"
        "REGLE ABSOLUE — MINIMUM 5 URLs HTTP(S) REELLES : le chapitre DOIT "
        "contenir au moins 5 lignes de sources avec une URL commencant par "
        "https:// (INSEE, Xerfi, Legifrance, Bpifrance, ministeres, "
        "publications reconnues, presse economique). Le gate compte et "
        "refuse la livraison si moins de 5 URLs sont trouvees. Une source "
        "sans URL reste acceptee UNIQUEMENT pour les documents client "
        "(« Fichier previsions.xlsx transmis par le porteur ») et NE COMPTE "
        "PAS dans les 5. Les formules « URL a confirmer », « lien "
        "indisponible », « source interne », « donnees non disponibles » "
        "sont INTERDITES : si tu n'as pas la source, tu ne cites pas la "
        "donnee dans le corps du document.\n"
        "FORMAT strict par puce, une URL par ligne :\n"
        "## Marche\n- Nom de la source, annee - https://www.editeur.fr/chemin\n"
        "## Demographie\n- ...\n"
