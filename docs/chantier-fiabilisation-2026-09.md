...
|---|---|---|---|
| A1 | EC : fourchette de CA/parts OBLIGATOIRE dans le prompt (`prompts.py` `exception_ca_estime`, EC ch 06), `fourchette_interdite` strict hors EM | EC | ⏳ |
| A2 | Plages produites par les consignes : `_SYSTEME` « entre X et Y », libellé du socle, « 14-16 % → 15 % retenu », `DECISIONS_STRATEGIE` « fourchette ou prix cible », STR ch 17 seuils | BP EC STR | ⏳ |
| A3 | Détecteur de fourchettes : « de 60 à 65 € » non vu (« à » accentué) ; « An 1 — 120 000 € », « Scénario 2 - 40 % » accusés à tort ; EM jamais contrôlé | tous | ⏳ |
| A4 | `calcul_faux` : « 250 abonnés à 19 € par mois, soit 57 000 € par an » déclaré faux (×12 sur le seul nombre qui précède « par mois ») — la forme que PRIX règles 5-6 demande | BP EC STR | ⏳ |
| A5 | `chiffres_hors_socle` punit les calculs que les prompts exigent de montrer ; dérivations coupées à 80 valeurs dont les doublons | tous | ✅ calcul posé et chiffre sourcé reconnus dans leur phrase ; doublons retirés avant la coupe |
