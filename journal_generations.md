...
| 2026-09-11 | `8ad03a60` | `f5900bc` | STR | 4,46 | 33 | 4 | en attente | **blocked** | Zenitek, reprise « sans envoi » du dossier `f7f2fad9`, PREMIER dossier généré après la lecture des documents déposés. Cinq documents lus (brief Word, deux études PDF, business plan PDF, prévisionnel Excel). **Les trois reproches de la cliente ont disparu du document** : plus de « 900 M€ » sans source (remplacé par 96 000 M€ sur un périmètre nommé, avec la réserve que le sous-segment n'est pas mesuré), plus d'« objectif » de 3 000 € qui était le chiffre d'affaires actuel, plus d'intertitre « Ce que ce chapitre change ». Le document distingue désormais l'ensemble DIBITEK (120 000 € de CA, 38 % de récurrent — chiffres venus des pièces jointes) de la seule activité Zenitek, et discute les trois paliers d'abonnement (12/19/29 €), ce que la cliente réclamait. 68 mentions « données du projet » en source, aucun nom de fichier dans le texte. **Trois mesures nouvelles** : (1) les 40 000 caractères transmis se partagent entre les cinq documents, soit ~8 000 chacun — l'étude de marché n'est lue que sur son premier cinquième ; (2) le contrôle des sources exigeait la moitié des sources avec une URL et comptait les données du client dans le total : il tombe désormais sur tout document bien fondé sur les pièces jointes — corrigé, le ratio ne porte plus que sur les sources extérieures ; (3) l'avertissement « cellules calculées sans valeur » sortait une fois par feuille au lieu d'un total. Restent 3 motifs de gate à instruire : fourchette « 60-65 € » (ch. 10), canaux à éviter non posés (ch. 13), sujet déclaré « non traité » alors qu'il est traité ailleurs (ch. 5). |

| 2026-09-12 | `db0d9508` | `0cbe52c`→`10400d9` | STR | 6,51 | 34 | 2 | en attente | **discard** | Zenitek, reprise « sans envoi » de `b098ded3`, premier dossier après les blocs de prompt du 12/09 (cohérence chiffrée, sources, catalogue des figures, validation des figures à l'acceptation). **Deux chapitres MORTS**, dont `str.20.sources`, sur le contrôle du vocabulaire interne : l'en-tête du bloc de chiffres écrivait « SOCLE VERROUILLÉ » en capitales et la consigne listait en toutes lettres les locutions qu'elle interdisait. Document amputé du ch. 12, retenu, dossier en « intervention requise » — ce que la cliente a lu comme une demande qu'on ne pouvait pas satisfaire. Figures 11/18 (61 %) : moins demandées, mieux choisies. Mais **34 reprises** contre 4 sur `655b0908`. Réponse : `45af2ff` (prompt sans le vocabulaire puni, test de classe sur `_VOCABULAIRE_INTERNE` ; chapitre manquant refait par le contrôleur final). |

| 2026-09-12 | `db228221` | `45af2ff` | STR | 6,52 | 34 | 0 | en attente | **discard** | Zenitek, reprise « sans envoi » de `b098ded3` après `45af2ff`. **21/21 chapitres, aucun mort** : le correctif du vocabulaire tient sur un vrai dossier. Chiffres hors socle : **0**. Anomalies restantes 19 (contre 28 à 36 sur les quatre dossiers Zenitek précédents). Figures 7/17 (41 %), dix refusées pour exactement les motifs que la validation d'acceptation devait arrêter. **Le défaut de ce dossier est un coût** : chapitres retenus 2,54 €, total 6,52 € — **3,98 € de tentatives jetées (61 %)**, 34 reprises, contre 0,36 € et 4 reprises sur `655b0908`. Cause : la validation des figures du 12/09 relançait un chapitre entier pour une figure impossible, puis la laissait passer au dernier essai. Budget vidé ⇒ la relecture finale s'est arrêtée sur « budget du dossier épuisé » sans corriger aucun des 7 défauts réparables qu'elle avait trouvés (5 densité, 2 zéros nus). Sources : 2 extérieures, 2 sans adresse, 0 du client — inchangé sur six dossiers ; la mesure ne disait pas encore combien d'adresses la recherche web avait rapportées. Réponse : une figure impossible ne paie plus jamais une reprise à elle seule ; `adresses_collectees` ajouté à la mesure. |

| 2026-09-13 | `a678b10a` | `a325da4` | STR | 6,41 | 2 | 0 | en attente | **blocked** | Zenitek, reprise « sans envoi » de `b098ded3`, DERNIÈRE génération autorisée (le client a interdit toute nouvelle génération après celle-ci). Premier dossier avec la recherche web de Claude (`web_search_20250305`). **Les sources sont réparées** : 46 adresses rapportées (0 sur les six dossiers Zenitek précédents), chapitre Sources à **10 sources extérieures, 0 sans adresse**, 4 données du projet. **Le correctif des reprises tient** : 21/21 chapitres, 2 reprises (34 sur `db228221`), tentatives jetées ~0,37 € (3,98 €). Chiffres hors socle : 2 (« 6,63 % » ch. 4, « 62,3 % » ch. 15 — des ratios calculés non posés). **Deux défauts restent, et ils sont nommés.** (1) **Figures 6/29 (20 %)**, 23 repliées en tableau : sans reprise pour les punir, le modèle en redemande autant qu'avant le catalogue, pour les mêmes motifs (unités hétérogènes, radars et jauges sans notes, frises sans dates, figures à un seul chiffre). Le catalogue fermé du prompt n'est PAS obéi ; seule une contrainte de schéma rendra la figure impossible inexprimable. (2) **Deux correcteurs en série** : la boucle de correction du gate (héritée) a réécrit les chapitres 8, 10, 13, 19 et 20 pendant 18 minutes pour ~2 € — sans résoudre ses trois points (fourchette « 60-75 € » ch. 10, deux piliers sans décision) — puis la relecture finale a trouvé le budget vide et s'est arrêtée (« budget du dossier épuisé », 0 chapitre réécrit, 30 anomalies laissées). Coût de la recherche : ~1,30 € au barème du projet (307 459 jetons d'entrée, 37 325 de sortie sur le chapitre 0), soit le double de l'estimation : les synthèses par requête sont trop longues. |
